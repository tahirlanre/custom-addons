...
class Parser(report_sxw.rml_parse):
    ...
    def _stock_move_in_out(self, move, form):
        source_in = ["supplier", "inventory", "customer"]
        source_out = ["internal"]
        stock_in = 0
        stock_out = 0

        if move.location_dest_id.id == form["location_id"][0]:
    ...
    def _stock_moves(self, form):
        ...
        move_ids = move_obj.search(
            ...
            self.uid,
            [
                ("date", ">=", form["date_from"] + " 00:00:00"),
                ("date", "<=", form["date_to"] + " 23:59:59"),
                ("product_id", "=", form["product_id"][0]),
                "|",
                ("location_id", "=", form["location_id"][0]),
                ("location_dest_id", "=", form["location_id"][0]),
    ...
    def get_move_accumulated_balance(self, form, move):
        ...
        for m in stock_moves:
            accumulated_balance += self._get_stock_move_in(
                m, form
            ) - self._get_stock_move_out(m, form)
            if m == move:
                break
        return accumulated_balance

    def _get_stock_move_out(self, move, form):
        return self._stock_move_in_out(move, form)[1]

    def _get_stock_move_in(self, move, form):
        return self._stock_move_in_out(move, form)[0]

    def _stock_move_out_to_print(self, move, form):
        stock_out = self._stock_move_in_out(move, form)[1]
    ...
    def _stock_move_in_to_print(self, move, form):
        stock_in = self._stock_move_in_out(move, form)[0]
    ...
    def _qty_available(self, form):
        ...
        move_in = move_obj.search(
            ...
            [
                ...
                ("product_id", "=", form["product_id"][0]),
                ("location_dest_id", "=", form["location_id"][0]),
                # ("location_dest_id.usage", "not in", source_in),
                ("state", "=", "done"),
            ],
        )
        move_out = move_obj.search(
            self.cr,
            self.uid,
            [
                ("date", "<=", form["date_to"] + " 23:59:59"),
                ("product_id", "=", form["product_id"][0]),
                ("location_id", "=", form["location_id"][0]),
                # ("location_dest_id.usage", "not in", source_out),
                ("state", "=", "done"),
            ],
        )
        total_in = 0
